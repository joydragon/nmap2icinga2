...
function clean_port_exception(){
	local PORT_LIST=$1
	FINAL=""

	while IFS= read -r PORT
	do
		RES=`echo $PORT_EXCEPTIONS | grep -e ";${PORT};"`
		if [[ -z $RES ]];then
			#echo "PUERTO $PORT"
			FINAL="${FINAL}${PORT}\n"
		fi
...
function check_if_ip_configured(){
	...
	if [[ -n $OUT ]]; then
		...
		CURL_CONF="curl -s -k -H 'Accept: application/json' -H 'X-HTTP-Method-Override: GET' -u $CURL_USER_AUTH \"${CURL_ICINGA_HOST}/v1/objects/hosts?host=""$IP""\" -d '{ \"attrs\" : [ \"name\", \"vars\", \"source_location\" ] }' | jq -r '.results[0].name' | grep -v 'null'"
		IS_CONF=$(eval "$CURL_CONF")

		if [[ -n $IS_CONF ]]; then
			# Si la IP esta arriba y se tiene en la configuracion se sacan los puertos
			CURL_PORTS="curl -s -k -H 'Accept: application/json' -H 'X-HTTP-Method-Override: GET' -u $CURL_USER_AUTH \"${CURL_ICINGA_HOST}/v1/objects/hosts?host=""$IP""\" -d '{ \"attrs\" : [ \"name\", \"vars\", \"source_location\" ] }' | jq -r '.results[0].attrs.vars.ports' | grep -v -e '\[' -e '\]' | sed -re 's/(\"|,|\s+)//g'"
			eval "$CURL_PORTS" > $TMP_P1
			...
			RES2=`diff $TMP_P1 $TMP_P2 | grep "^>" | sed -e "s/^>\s*//"`

			MISSING_PORTS=$(clean_port_exception "$RES1")
			NEW_PORTS=$(clean_port_exception "$RES2")
			...
			RES1=`xmlstarlet sel -t -m "/nmaprun/host[address/@addr='"$IP"']" -m "ports/port[state/@state='open']" -v "@portid" -n $ARCHIVO_OUTPUT | grep -v "^$"`
			NEW_PORTS=$(clean_port_exception "$RES1")
